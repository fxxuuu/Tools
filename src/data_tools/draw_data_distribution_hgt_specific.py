...
with open('/home/fangxin/fxx_waist_workspace/dataset/upground/with_ignore/training_format_data/human_waist_4_upground_10w_with_ignore.data', 'r') as fp:
    data = {'data': []}
    lines = fp.readlines()
    length = len(lines)
    print length
...
for idx, idata in enumerate(data['data']):
    ...
    for gt_box in gt_boxes:
        x1, y1, x2, y2, waist = gt_box
        if waist == -1.0:
            invalid_gt_box_count += 1
        rect_height = min(int((y2-y1)/100), 10)
        gt_scale_map[rect_height] += 1
        gt_box_count += 1

    for gt_ignore_box in gt_ignore_boxes:
        x1, y1, x2, y2, waist = gt_ignore_box
        rect_height = min(int(y2-y1)/100, 10)
        gt_ignore_scale_map[rect_height] += 1
        gt_ignore_box_count += 1

draw_bar(scale_label, gt_scale_map, '/home/fangxin/fxx_waist_workspace/dataset/upground_10w_gt_data_distribution.png')
print "\n", gt_scale_map
draw_bar(scale_label, gt_ignore_scale_map, '/home/fangxin/fxx_waist_workspace/dataset/upground_10w_gt_ignore_distribution.png')
